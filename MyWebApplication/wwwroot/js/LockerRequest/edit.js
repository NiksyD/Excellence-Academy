// LockerRequest Edit Form File Upload Functionality
...
function initializeFormValidation() {
    const form = document.getElementById('editLockerRequestForm');
...
// Delete document function for existing files
function deleteDocument(documentId) {
    if (confirm('Are you sure you want to delete this document? This action cannot be undone.')) {
        fetch(`/LockerRequest/DeleteDocument/${documentId}`, {
            method: 'DELETE',
            headers: {
        ...
        .catch(error => {
            ...
            showErrorMessage('Error deleting document: ' + error.message);
        });
    }
}
...
function showAlert(message, type) {
    // Remove existing alerts
    const existingAlerts = document.querySelectorAll('.document-alert');
    existingAlerts.forEach(alert => alert.remove());

    // Create new alert
    const alertDiv = document.createElement('div');
    alertDiv.className = `alert alert-${type} alert-dismissible fade show document-alert mt-3`;
    alertDiv.innerHTML = `
        <i class="fas fa-${type === 'success' ? 'check-circle' : 'exclamation-triangle'} me-2"></i>
        ${message}
        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
    ...
    // Insert at the top of the existing files container
    const existingFilesContainer = document.querySelector('.existing-files-container');
    if (existingFilesContainer) {
        existingFilesContainer.parentNode.insertBefore(alertDiv, existingFilesContainer);
    }
